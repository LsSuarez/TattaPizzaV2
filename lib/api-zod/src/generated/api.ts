/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Api
 * Tata Pizza API - Sistema de automatización de pedidos con portal cliente
 * OpenAPI spec version: 0.2.0
 */
import * as zod from "zod";

/**
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  status: zod.string(),
});

/**
 * @summary List all menu items
 */
export const ListMenuItemsQueryParams = zod.object({
  category: zod.coerce.string().optional(),
  available: zod.coerce.boolean().optional(),
});

export const ListMenuItemsResponseItem = zod.object({
  id: zod.number(),
  name: zod.string(),
  description: zod.string().nullish(),
  category: zod.string(),
  priceSmall: zod.number().nullish(),
  priceMedium: zod.number().nullish(),
  priceLarge: zod.number().nullish(),
  available: zod.boolean(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});
export const ListMenuItemsResponse = zod.array(ListMenuItemsResponseItem);

/**
 * @summary Create a menu item
 */
export const CreateMenuItemBody = zod.object({
  name: zod.string(),
  description: zod.string().nullish(),
  category: zod.string(),
  priceSmall: zod.number().nullish(),
  priceMedium: zod.number().nullish(),
  priceLarge: zod.number().nullish(),
  available: zod.boolean().optional(),
});

/**
 * @summary Get a menu item by ID
 */
export const GetMenuItemParams = zod.object({
  id: zod.coerce.number(),
});

export const GetMenuItemResponse = zod.object({
  id: zod.number(),
  name: zod.string(),
  description: zod.string().nullish(),
  category: zod.string(),
  priceSmall: zod.number().nullish(),
  priceMedium: zod.number().nullish(),
  priceLarge: zod.number().nullish(),
  available: zod.boolean(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});

/**
 * @summary Update a menu item
 */
export const UpdateMenuItemParams = zod.object({
  id: zod.coerce.number(),
});

export const UpdateMenuItemBody = zod.object({
  name: zod.string().optional(),
  description: zod.string().nullish(),
  category: zod.string().optional(),
  priceSmall: zod.number().nullish(),
  priceMedium: zod.number().nullish(),
  priceLarge: zod.number().nullish(),
  available: zod.boolean().optional(),
});

export const UpdateMenuItemResponse = zod.object({
  id: zod.number(),
  name: zod.string(),
  description: zod.string().nullish(),
  category: zod.string(),
  priceSmall: zod.number().nullish(),
  priceMedium: zod.number().nullish(),
  priceLarge: zod.number().nullish(),
  available: zod.boolean(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});

/**
 * @summary Delete a menu item
 */
export const DeleteMenuItemParams = zod.object({
  id: zod.coerce.number(),
});

/**
 * @summary List orders
 */
export const ListOrdersQueryParams = zod.object({
  status: zod.coerce.string().optional(),
  limit: zod.coerce.number().optional(),
});

export const ListOrdersResponseItem = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );
export const ListOrdersResponse = zod.array(ListOrdersResponseItem);

/**
 * @summary Create a new order
 */
export const CreateOrderBody = zod.object({
  customerName: zod.string(),
  customerPhone: zod.string(),
  customerEmail: zod.string().nullish(),
  deliveryAddress: zod.string().nullish(),
  deliveryType: zod.string().optional(),
  paymentMethod: zod.string().nullish(),
  notes: zod.string().nullish(),
  whatsappMessageId: zod.string().nullish(),
  items: zod.array(
    zod.object({
      menuItemId: zod.number().nullish(),
      name: zod.string(),
      size: zod.string().nullish(),
      quantity: zod.number(),
      unitPrice: zod.number(),
    }),
  ),
});

/**
 * @summary Get an order by ID
 */
export const GetOrderParams = zod.object({
  id: zod.coerce.number(),
});

export const GetOrderResponse = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );

/**
 * @summary Update order status
 */
export const UpdateOrderStatusParams = zod.object({
  id: zod.coerce.number(),
});

export const UpdateOrderStatusBody = zod.object({
  status: zod.string(),
  notes: zod.string().nullish(),
});

export const UpdateOrderStatusResponse = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );

/**
 * @summary Update order payment (submit proof or verify)
 */
export const UpdateOrderPaymentParams = zod.object({
  id: zod.coerce.number(),
});

export const UpdateOrderPaymentBody = zod.object({
  paymentStatus: zod.string(),
  paymentProofUrl: zod.string().nullish(),
});

export const UpdateOrderPaymentResponse = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );

/**
 * @summary Submit a rating for an order
 */
export const CreateOrderRatingParams = zod.object({
  id: zod.coerce.number(),
});

export const CreateOrderRatingBody = zod.object({
  productRating: zod.number(),
  deliveryRating: zod.number(),
  serviceRating: zod.number(),
  comment: zod.string().nullish(),
});

/**
 * @summary List customers
 */
export const ListCustomersQueryParams = zod.object({
  phone: zod.coerce.string().optional(),
});

export const ListCustomersResponseItem = zod.object({
  id: zod.number(),
  clerkUserId: zod.string().nullish(),
  name: zod.string(),
  email: zod.string().nullish(),
  phone: zod.string(),
  address: zod.string().nullish(),
  notes: zod.string().nullish(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});
export const ListCustomersResponse = zod.array(ListCustomersResponseItem);

/**
 * @summary Create or upsert a customer
 */
export const CreateCustomerBody = zod.object({
  name: zod.string(),
  email: zod.string().nullish(),
  phone: zod.string(),
  address: zod.string().nullish(),
  notes: zod.string().nullish(),
});

/**
 * @summary Get a customer by ID
 */
export const GetCustomerParams = zod.object({
  id: zod.coerce.number(),
});

export const GetCustomerResponse = zod.object({
  id: zod.number(),
  clerkUserId: zod.string().nullish(),
  name: zod.string(),
  email: zod.string().nullish(),
  phone: zod.string(),
  address: zod.string().nullish(),
  notes: zod.string().nullish(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});

/**
 * @summary Update a customer
 */
export const UpdateCustomerParams = zod.object({
  id: zod.coerce.number(),
});

export const UpdateCustomerBody = zod.object({
  name: zod.string().optional(),
  email: zod.string().nullish(),
  address: zod.string().nullish(),
  notes: zod.string().nullish(),
});

export const UpdateCustomerResponse = zod.object({
  id: zod.number(),
  clerkUserId: zod.string().nullish(),
  name: zod.string(),
  email: zod.string().nullish(),
  phone: zod.string(),
  address: zod.string().nullish(),
  notes: zod.string().nullish(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});

/**
 * @summary Get orders for a specific customer
 */
export const GetCustomerOrdersParams = zod.object({
  id: zod.coerce.number(),
});

export const GetCustomerOrdersResponseItem = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );
export const GetCustomerOrdersResponse = zod.array(
  GetCustomerOrdersResponseItem,
);

/**
 * @summary Get authenticated customer profile
 */
export const GetMyProfileResponse = zod.object({
  id: zod.number().nullish(),
  clerkUserId: zod.string().nullish(),
  name: zod.string().nullish(),
  email: zod.string().nullish(),
  phone: zod.string().nullish(),
  address: zod.string().nullish(),
  isProfileComplete: zod.boolean(),
});

/**
 * @summary Create or update customer profile for authenticated user
 */
export const UpsertMyProfileBody = zod.object({
  name: zod.string(),
  email: zod.string().nullish(),
  phone: zod.string(),
  address: zod.string().nullish(),
});

export const UpsertMyProfileResponse = zod.object({
  id: zod.number().nullish(),
  clerkUserId: zod.string().nullish(),
  name: zod.string().nullish(),
  email: zod.string().nullish(),
  phone: zod.string().nullish(),
  address: zod.string().nullish(),
  isProfileComplete: zod.boolean(),
});

/**
 * @summary Get authenticated customer's orders
 */
export const GetMyOrdersResponseItem = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );
export const GetMyOrdersResponse = zod.array(GetMyOrdersResponseItem);

/**
 * @summary Create an order for authenticated customer
 */
export const CreateMyOrderBody = zod.object({
  deliveryAddress: zod.string().nullish(),
  deliveryType: zod.string(),
  paymentMethod: zod.string(),
  notes: zod.string().nullish(),
  items: zod.array(
    zod.object({
      menuItemId: zod.number().nullish(),
      name: zod.string(),
      size: zod.string().nullish(),
      quantity: zod.number(),
      unitPrice: zod.number(),
    }),
  ),
});

/**
 * @summary Get a specific order for authenticated customer
 */
export const GetMyOrderParams = zod.object({
  id: zod.coerce.number(),
});

export const GetMyOrderResponse = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );

/**
 * @summary Submit payment proof for an order
 */
export const SubmitMyPaymentParams = zod.object({
  id: zod.coerce.number(),
});

export const SubmitMyPaymentBody = zod.object({
  paymentMethod: zod.string(),
  paymentProofUrl: zod.string().nullish(),
});

export const SubmitMyPaymentResponse = zod
  .object({
    id: zod.number(),
    customerId: zod.number().nullish(),
    clerkUserId: zod.string().nullish(),
    customerName: zod.string(),
    customerPhone: zod.string(),
    customerEmail: zod.string().nullish(),
    deliveryAddress: zod.string().nullish(),
    deliveryType: zod.string(),
    status: zod.string(),
    paymentMethod: zod.string().nullish(),
    paymentStatus: zod.string().nullish(),
    paymentProofUrl: zod.string().nullish(),
    total: zod.number(),
    notes: zod.string().nullish(),
    whatsappMessageId: zod.string().nullish(),
    rating: zod
      .object({
        id: zod.number(),
        orderId: zod.number(),
        customerId: zod.number().nullish(),
        clerkUserId: zod.string().nullish(),
        productRating: zod.number(),
        deliveryRating: zod.number(),
        serviceRating: zod.number(),
        comment: zod.string().nullish(),
        createdAt: zod.coerce.date(),
      })
      .nullish(),
    createdAt: zod.coerce.date(),
    updatedAt: zod.coerce.date(),
  })
  .and(
    zod.object({
      items: zod.array(
        zod.object({
          id: zod.number(),
          orderId: zod.number(),
          menuItemId: zod.number().nullish(),
          name: zod.string(),
          size: zod.string().nullish(),
          quantity: zod.number(),
          unitPrice: zod.number(),
          subtotal: zod.number(),
          createdAt: zod.coerce.date(),
        }),
      ),
    }),
  );

/**
 * @summary Submit rating for a delivered order
 */
export const SubmitMyRatingParams = zod.object({
  id: zod.coerce.number(),
});

export const SubmitMyRatingBody = zod.object({
  productRating: zod.number(),
  deliveryRating: zod.number(),
  serviceRating: zod.number(),
  comment: zod.string().nullish(),
});

/**
 * @summary Get authenticated customer's addresses
 */
export const GetMyAddressesResponseItem = zod.object({
  id: zod.number(),
  customerId: zod.number(),
  clerkUserId: zod.string().nullish(),
  label: zod.string(),
  address: zod.string(),
  district: zod.string().nullish(),
  reference: zod.string().nullish(),
  instructions: zod.string().nullish(),
  isDefault: zod.boolean(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});
export const GetMyAddressesResponse = zod.array(GetMyAddressesResponseItem);

/**
 * @summary Add a new address
 */
export const CreateMyAddressBody = zod.object({
  label: zod.string(),
  address: zod.string(),
  district: zod.string().nullish(),
  reference: zod.string().nullish(),
  instructions: zod.string().nullish(),
  isDefault: zod.boolean().optional(),
});

/**
 * @summary Update an address
 */
export const UpdateMyAddressParams = zod.object({
  id: zod.coerce.number(),
});

export const UpdateMyAddressBody = zod.object({
  label: zod.string().optional(),
  address: zod.string().optional(),
  district: zod.string().nullish(),
  reference: zod.string().nullish(),
  instructions: zod.string().nullish(),
  isDefault: zod.boolean().optional(),
});

export const UpdateMyAddressResponse = zod.object({
  id: zod.number(),
  customerId: zod.number(),
  clerkUserId: zod.string().nullish(),
  label: zod.string(),
  address: zod.string(),
  district: zod.string().nullish(),
  reference: zod.string().nullish(),
  instructions: zod.string().nullish(),
  isDefault: zod.boolean(),
  createdAt: zod.coerce.date(),
  updatedAt: zod.coerce.date(),
});

/**
 * @summary Delete an address
 */
export const DeleteMyAddressParams = zod.object({
  id: zod.coerce.number(),
});

/**
 * @summary Get dashboard statistics
 */
export const GetDashboardStatsResponse = zod.object({
  totalOrdersToday: zod.number(),
  pendingOrders: zod.number(),
  inPreparationOrders: zod.number(),
  readyOrders: zod.number(),
  deliveredOrdersToday: zod.number(),
  totalRevenueToday: zod.number(),
  totalCustomers: zod.number(),
  topItems: zod.array(
    zod.object({
      name: zod.string(),
      count: zod.number(),
    }),
  ),
});

/**
 * @summary Webhook for n8n WhatsApp integration
 */
export const WhatsappWebhookBody = zod.object({
  customerName: zod.string(),
  customerPhone: zod.string(),
  deliveryAddress: zod.string().nullish(),
  deliveryType: zod.string().optional(),
  notes: zod.string().nullish(),
  whatsappMessageId: zod.string().nullish(),
  items: zod.array(
    zod.object({
      name: zod.string(),
      size: zod.string().nullish(),
      quantity: zod.number(),
      unitPrice: zod.number(),
    }),
  ),
});

export const WhatsappWebhookResponse = zod.object({
  success: zod.boolean(),
  orderId: zod.number(),
  message: zod.string(),
  order: zod
    .object({
      id: zod.number(),
      customerId: zod.number().nullish(),
      clerkUserId: zod.string().nullish(),
      customerName: zod.string(),
      customerPhone: zod.string(),
      customerEmail: zod.string().nullish(),
      deliveryAddress: zod.string().nullish(),
      deliveryType: zod.string(),
      status: zod.string(),
      paymentMethod: zod.string().nullish(),
      paymentStatus: zod.string().nullish(),
      paymentProofUrl: zod.string().nullish(),
      total: zod.number(),
      notes: zod.string().nullish(),
      whatsappMessageId: zod.string().nullish(),
      rating: zod
        .object({
          id: zod.number(),
          orderId: zod.number(),
          customerId: zod.number().nullish(),
          clerkUserId: zod.string().nullish(),
          productRating: zod.number(),
          deliveryRating: zod.number(),
          serviceRating: zod.number(),
          comment: zod.string().nullish(),
          createdAt: zod.coerce.date(),
        })
        .nullish(),
      createdAt: zod.coerce.date(),
      updatedAt: zod.coerce.date(),
    })
    .and(
      zod.object({
        items: zod.array(
          zod.object({
            id: zod.number(),
            orderId: zod.number(),
            menuItemId: zod.number().nullish(),
            name: zod.string(),
            size: zod.string().nullish(),
            quantity: zod.number(),
            unitPrice: zod.number(),
            subtotal: zod.number(),
            createdAt: zod.coerce.date(),
          }),
        ),
      }),
    ),
});
